<nav class="bg-transparent px-2  border-gray-200 fixed top-0 z-10 w-full" x-data="{
  ...
  <div class="menu mx-auto flex">
    <a href="/" class="flex ">
      <img src="{{asset('img/logotipo_blanco.png')}}" class="mt-3 ml-8 lg:h-20 h-20 md:h-20 sm:h-20" alt="Logo">
    </a>
    ...
    <div :class="{
      ...
      <ul class="flex items-center p-4">
        @foreach ($items as $item)
          <li @class([
              'ml-auto mr-5' => $loop->iteration == 5,
              'mr-5' => $loop->iteration <= 3,
              'bg-green-700 hover:bg-green-500 text-white font-bold py-2 px-4 rounded' => $loop->iteration == 6
            ])>
